...
```{r}
#Set the URL
theURL1 <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02087500&period=24&format=rdb'
...
```{r}
#Apply the API key
census_api_key("d0be50201868fd3d78e8f7967dbe826ecd3ad65e.", install = TRUE)
...
the_states <- the_website %>% html_nodes("tr :nth-child(1)") %>% 
  html_text()
the_price <- the_website %>% html_nodes("tr :nth-child(2)") %>% 
  html_text()
the_capacity <- the_website %>% html_nodes("tr :nth-child(3)") %>% 
  html_text()
net_generation <- the_website %>% html_nodes("tr :nth-child(4)") %>% 
  html_text()
total_retail <- the_website %>% html_nodes("tr :nth-child(5)") %>% 
  html_text()
  
#3 Construct a dataframe from the values
 df_nc <- data.frame("state' = the_states,
 "price" = as.numeric(the_price)
 "capcity" = as.numeric(gsub)
 rep(1:12),
