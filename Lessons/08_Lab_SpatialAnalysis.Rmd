...
EPA_Wind_Sf <- st_read("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c("HUC_12" = "factor")) %>% 
  mutate(HUC_8 = substr(Huc_12,0, 8)) %>% 
  group_by(HUC_8)
  summarise(SumWindEnergy = sum(avg(WindEnegry)))


#Join to HUC_8 features
EPA_Wind_Join <-  merge(x = Huc_8,
                           y = EPA_Wind_Sf, 
                           by.x = "HUC_8", 
                           by.y = "HUC_8")

EPA_Wind_Join <- EPA_Wind_Sf %>% left_join()
...
huc2_utm <-        st_transform(huc2, crs = 26917)
```
