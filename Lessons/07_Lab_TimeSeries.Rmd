...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # Seperate fct seperate in two 
  mutate( Date = my(paste0(Month,"-",Year))) %>%   #pasting all the stuffs together 
...
Rainfall_Plot <- ggplot(BRA_complete, aes(Date, Rainfall)) +
  geom_line() +
  geom_smooth(method = lm)

Temp_Plot <- ggplot(BRA_complete, aes(Date, Temperature_C)) +
  geom_line() +
  geom_smooth(method = lm)

Plot_2 <- plot_grid(Rainfall_Plot, Temp_Plot, ncol = 1)
print(Plot_2)
...
BRA_complete_ts_R <- ts(BRA_complete$Rainfall, start=c(1, 1901),
                   frequency=12)

BRA_complete_ts_T <- ts(BRA_complete$Temperature_C, start=c(1, 1901),
                   frequency=12)
```
...
BRA_R_dec <- stl(BRA_complete_ts_R, s.window = "periodic")


BRA_T_dec <- stl(BRA_complete_ts_T,s.window = "periodic")
plot(BRA_R_dec)
plot(BRA_T_dec)
