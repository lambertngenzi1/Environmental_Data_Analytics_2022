...
PeterPaulPlot2 <- ggplot(subset(PeterPaul.chem.nutrients), aes(daynum, temperature_C)) + geom_point(aes(color= depth)) + (size = 0.5) + ylab("surface in Temperature") + scale_color_viridis(option = "magma", direction = -1)
...
PeterPaulPlot2 <- ggplot(PeterPaul.chem.nutrients, depth==0), aes(sampledate, temperature_C) + geom_point(aes(color= magma)) + facet_wrap(vars(lakename), nrow = 2) + ylab("surface in Temperature")
plot(PeterPaulPlot2)
