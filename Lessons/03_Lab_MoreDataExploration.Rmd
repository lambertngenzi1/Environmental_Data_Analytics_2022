...
```{r}
LowTemp <- c(30, 18, 26, 26, 19, 26, 26, 28,39, 44)
LowTemp
HighTemp <- c(58,39,41, 44,37,41,51,52,53,60)
HighTemp
```
...
LT_Celsius <- Fahrenheit_to_Celsius(LowTemp)
LT_Celsius
HT_Celsius <- Fahrenheit_to_Celsius(HighTemp)
HT_Celsius
```
...
```{r}
Durham_Temperature1 <- data.frame(LT_Celsius, HT_Celsius, LowTemp, HighTemp)
Durham_Temperature1
```
...
```{r}
summary(Durham_Temperature1)

sd('Durham_Temperature1')
```
...
>>>>>>> 58d54d286c74a22f4bcc8de1eb0d5d8b92cd6f10
#format(today, format = "")
